...
**Status:** Last checkpoint 2026-09-16 · Next: Day 6 - Commodity leg for NISM XVI: same engine on a commodity series, with contango and backwardation handled explicitly rather than assumed away
...
|------|--------|--------------|------|
| 2026-09-16 | `44e6da7` | Day 5: portfolio risk - VaR/CVaR at 95/99 by three methods (mcsim/risk.py, mcsim/returns.py), wired into a new mcsim.portfolio CLI alongside a Monte Carlo max-drawdown distribution and a Kupiec proportion-of-failures backtest. Historical, parametric and MC VaR/CVaR all read the same fitted sample from a real fixture (fixtures/ohlcv/RELIANCE_NS.csv, copied from STOCKSTALKER's committed NSE history) so a gap between methods is a modeling difference, not different data; MC uses the physical drift fitted from history, not the risk-neutral rate Days 1-4 price under, since there is no hedge here, only a real-world loss to estimate. At horizon=20d, N=1e6, seed=42: historical/parametric/MC VaR-CVaR are -0.0915/-0.1082, -0.1012/-0.1257, -0.1028/-0.1273 at 95pct and -0.1207/-0.1270, -0.1411/-0.1609, -0.1427/-0.1626 at 99pct. The Kupiec backtest of the 1-day parametric VaR against the full historical sample is not rejected at 95pct (20/500 exceptions vs 25 expected, p=0.29) but IS rejected at 99pct (10/500 vs 5 expected, p=0.048) - reported honestly, CLI exits 1 with a stderr note rather than hiding it, exactly the thin-GBM-tails failure mode the README has warned about since Day 1. Drawdown distribution (same 1e6 paths): mean -5.96pct, median -5.42pct, worst -25.02pct across paths, vs a single realized -23.87pct drawdown in the fixture window itself. README records the overlapping-window historical estimator's understated sampling uncertainty, the in-sample nature of the Kupiec test, and that only VaR/CVaR (not the drawdown distribution) gets a backtest this round, as open limitations. 31 new tests pass (123/123 total); CLI run by hand at N=1e6 with a drawdown-plot PNG generated and inspected. | Day 6 - Commodity leg for NISM XVI: same engine on a commodity series, with contango and backwardation handled explicitly rather than assumed away |
